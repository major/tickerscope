...
from tickerscope._parsing import parse_watchlist_names_response
from tickerscope._models import WatchlistSummary


def test_parse_watchlist_names_response(watchlist_names_response) -> None:
    """Parse fixture into WatchlistSummary list with correct field values."""
    result = parse_watchlist_names_response(watchlist_names_response)
...
def test_parse_watchlist_names_empty() -> None:
    ...
    result = parse_watchlist_names_response({"data": {"watchlists": []}})
...
def test_parse_watchlist_names_graphql_errors() -> None:
    ...
    with pytest.raises(APIError):
        parse_watchlist_names_response({"errors": [{"message": "unauthorized"}]})
